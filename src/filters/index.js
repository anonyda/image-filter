...
export function applyVignette(imageData, strength = 0.5) {
  const width = imageData.width;
  const height = imageData.height;
  const pixels = imageData.data;
...
export function applyColorTint(imageData, r = 0, g = 0, b = 0) {
  const pixels = imageData.data;
...
export function enhanceContrast(imageData, factor = 1.5) {
  const pixels = imageData.data;
